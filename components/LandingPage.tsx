...
const AvatarImage: React.FC<{ src: string; alt: string; className?: string }> = ({ src, alt, className }) => {
    ...
    const useFallback = () => {
        // DiceBear "Lorelei" style provides friendly, high-quality illustrated avatars
        // We use the name as a seed so it's consistent (Ruby always looks like Ruby)
    ...
    <div className="min-h-screen bg-[#FFFBEB] font-sans text-gray-900 overflow-x-hidden relative selection:bg-yellow-200">
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <div className="space-y-8 z-10 text-center animate-in slide-in-from-left-10 duration-700 fade-in relative lg:text-center">
                      <div className="flex flex-wrap justify-center gap-3 mb-4">
                          ...
                          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#FACC15] text-black shadow-sm">
                              <Award className="w-3 h-3 text-black" /><span className="text-xs font-bold tracking-wide">Est. 2005</span>
                          </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 justify-center">
                          ...
                          <div className="flex items-center gap-4 px-6 py-4 bg-white/50 rounded-xl border border-white backdrop-blur-sm justify-center">
                              <Shield className="w-6 h-6 text-gray-400" /><div className="text-xs font-bold text-gray-500 text-left">{getTranslation(lang, 'cta_hipaa')}<br/><span className="text-green-600">{getTranslation(lang, 'trust_soc2')}</span></div>
                          </div>
                      </div>
                      <div className="pt-8 flex items-center justify-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-widest">
                          <span>Serving 1M+ Users</span><div className="w-1 h-1 bg-gray-300 rounded-full"></div><span>Global Coverage</span>
                      </div>
                  </div>
                  <div className="relative lg:h-[600px] w-full flex items-center justify-center mt-12 lg:mt-0 animate-in slide-in-from-right-10 duration-700 fade-in">
                      <div className="relative w-[280px] md:w-[350px] h-[400px] md:h-[500px] bg-black rounded-[40px] border-8 border-white shadow-2xl overflow-hidden transform rotate-0 md:rotate-[-3deg] md:hover:rotate-0 transition-transform duration-500 z-20">
                           ...
                           <div className="absolute top-6 left-6 right-6 flex justify-between items-center">
                                ...
                                <div className="bg-black/30 backdrop-blur-md p-2 rounded-full border border-white/10"><Signal className="w-4 h-4 text-green-400" /></div>
                           </div>
                           <div className="absolute bottom-8 left-6 right-6">
                                <div className="flex items-center gap-3 mb-4"><div className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center"><MessageCircle className="w-5 h-5 text-white" /></div><div className="h-1 bg-white/20 flex-1 rounded-full overflow-hidden"><div className="h-full bg-white w-1/2 animate-pulse"></div></div></div>
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce opacity-50"><ChevronDown className="w-6 h-6 text-gray-400" /></div>
          </div>
      </section>
      ...
      <section className="py-12 border-y border-yellow-100 bg-[#FFFBEB] overflow-hidden z-10 relative">
          <div className="relative flex w-full overflow-hidden">
              <div className="animate-marquee whitespace-nowrap flex items-center gap-20 px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
                  {[...pressLogos, ...pressLogos, ...pressLogos].map((Logo, i) => (<div key={i} className="inline-block flex-shrink-0"><Logo /></div>))}
              </div>
          </div>
      </section>
      ...
      <section className="py-24 bg-[#FFFBEB] text-gray-900 relative overflow-hidden z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
              <div className="text-center max-w-3xl mx-auto mb-16"><h2 className="text-3xl md:text-4xl font-black mb-6">{getTranslation(lang, 'science_title')}</h2><p className="text-gray-600 text-lg">{getTranslation(lang, 'science_subtitle')}</p></div>
              <div className="grid md:grid-cols-3 gap-8 text-center">
                  <div className="p-8 rounded-3xl bg-white border border-yellow-200 shadow-sm hover:shadow-xl transition-all"><div className="text-5xl font-black text-peutic-yellow mb-2">{getTranslation(lang, 'stat_1_val')}</div><p className="text-gray-500 font-bold">{getTranslation(lang, 'stat_1_desc')}</p></div>
                  <div className="p-8 rounded-3xl bg-white border border-yellow-200 shadow-sm hover:shadow-xl transition-all"><div className="text-5xl font-black text-peutic-yellow mb-2">{getTranslation(lang, 'stat_2_val')}</div><p className="text-gray-500 font-bold">{getTranslation(lang, 'stat_2_desc')}</p></div>
                  <div className="p-8 rounded-3xl bg-white border border-yellow-200 shadow-sm hover:shadow-xl transition-all"><div className="text-5xl font-black text-peutic-yellow mb-2">{getTranslation(lang, 'stat_3_val')}</div><p className="text-gray-500 font-bold">{getTranslation(lang, 'stat_3_desc')}</p></div>
              </div>
          </div>
      </section>
              ...
              <div className="grid md:grid-cols-2 gap-16 items-center">
                  <div className="order-2 md:order-1">
                       <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-gray-100 aspect-video md:aspect-auto md:h-[500px]">
                           <video src="https://player.vimeo.com/external/538560073.sd.mp4?s=8c2612484364310249855935916912919876764c&profile_id=164&oauth2_token_id=57447761" poster="https://images.unsplash.com/photo-1516321497487-e288fb19713f?q=80&w=1000&auto=format&fit=crop" className="w-full h-full object-cover" autoPlay muted loop playsInline></video>
                           <div className="absolute inset-0 bg-black/10"></div>
                           <div className="absolute bottom-8 left-8 text-white">
                               <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center mb-6 cursor-pointer hover:bg-white/30 transition-colors"><Play className="w-6 h-6 fill-white text-white ml-1" /></div>
                               <h3 className="text-2xl font-bold">See how it works</h3><p className="opacity-80">Watch a 30s demo session</p>
                           </div>
                       </div>
                  </div>
                  <div className="order-1 md:order-2 space-y-8">
                      ...
                      <div className="space-y-6">
                          <div className="flex gap-4 items-start"><div className="w-12 h-12 bg-yellow-50 rounded-xl flex items-center justify-center flex-shrink-0"><Clock className="w-6 h-6 text-yellow-600" /></div><div><h4 className="font-bold text-xl mb-2">{getTranslation(lang, 'comp_1')}</h4><p className="text-gray-500">No scheduling. No waiting rooms. Connect in under 30 seconds.</p></div></div>
                          <div className="flex gap-4 items-start"><div className="w-12 h-12 bg-yellow-50 rounded-xl flex items-center justify-center flex-shrink-0"><CreditCardIcon className="w-6 h-6 text-yellow-600" /></div><div><h4 className="font-bold text-xl mb-2">{getTranslation(lang, 'comp_2')}</h4><p className="text-gray-500">Pay $1.49/min. No monthly subscriptions or hidden fees.</p></div></div>
                          <div className="flex gap-4 items-start"><div className="w-12 h-12 bg-yellow-50 rounded-xl flex items-center justify-center flex-shrink-0"><Lock className="w-6 h-6 text-yellow-600" /></div><div><h4 className="font-bold text-xl mb-2">{getTranslation(lang, 'stat_3_desc')}</h4><p className="text-gray-500">Enterprise-grade security ensures your conversations stay private.</p></div></div>
                      </div>
                  </div>
              </div>
          </div>
      ...
      <section className="py-24 bg-[#FFFBEB] text-center z-10 relative">
          <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="bg-white rounded-3xl p-8 md:p-12 inline-block max-w-lg w-full relative overflow-hidden shadow-xl border border-yellow-200">
                  <div className="inline-block bg-red-600 text-white px-4 py-1.5 rounded-full text-xs font-black mb-6 animate-pulse tracking-widest">LIFETIME RATE LOCKED IN</div>
                  <p className="text-gray-500 font-bold uppercase tracking-widest mb-4">{getTranslation(lang, 'pricing_title')}</p>
                  <div className="flex items-baseline justify-center gap-4 mb-4"><span className="text-3xl text-gray-400 font-bold line-through decoration-red-500 decoration-2 opacity-70">$1.99</span><div className="text-6xl md:text-7xl font-black text-black flex items-start gap-1"><span className="text-3xl mt-2">$</span>1.49<span className="text-xl text-gray-400 mt-8">/min</span></div></div>
                  ...
                  <button onClick={() => onLoginClick(true)} className="w-full bg-[#FACC15] text-black py-4 rounded-xl font-bold hover:bg-[#EAB308] transition-transform hover:scale-105 shadow-lg shadow-yellow-500/20">Lock In $1.49 Rate Now</button>
              </div>
          </div>
      </section>

      {showCookies && <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-gray-200 p-6 z-[60]"><div className="max-w-7xl mx-auto flex justify-between items-center"><div className="flex items-center gap-4"><div className="p-3 bg-yellow-50 rounded-full"><Cookie className="w-6 h-6 text-yellow-600" /></div><p className="text-sm text-gray-600 max-w-2xl">We use encrypted cookies.</p></div><button onClick={acceptCookies} className="px-6 py-2 text-sm font-bold bg-black text-white rounded-lg">Accept</button></div></div>}
      
      <footer className="bg-[#FFFBEB] text-gray-900 py-12 border-t border-yellow-200 z-10 relative">
          <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="flex items-center gap-2"><Heart className="w-5 h-5 text-black" /><span className="font-bold">Peutic Inc.</span></div>
              <div className="flex gap-8 text-sm font-bold"><Link to="/privacy">Privacy</Link><Link to="/terms">Terms</Link><Link to="/support">Support</Link></div>
              <p className="text-xs opacity-50 font-medium">© 2025 Peutic Inc.</p>
          </div>
      </footer>
    </div>
  );
