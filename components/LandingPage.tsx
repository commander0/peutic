...
import { Shield, Heart, Star, Clock, CheckCircle, ArrowRight, Lock, Globe, ChevronDown, Play, MessageCircle, Signal, Cookie } from 'lucide-react';
import { LanguageCode, getTranslation } from '../services/i18n';
import { Link } from 'react-router-dom';

// --- STATIC SVG COMPONENTS (Fixed Glitching) ---
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="grid lg:grid-cols-2 gap-12 items-center">
                  <div className="space-y-8 z-10 text-center lg:text-left">
                      ...
                      <p className="text-lg md:text-xl text-gray-600 max-w-lg mx-auto lg:mx-0 leading-relaxed">{getTranslation(lang, 'hero_subtitle')}</p>
                  ...
                  <div className="relative lg:h-[600px] w-full flex items-center justify-center mt-12 lg:mt-0">
                      <div className="relative w-[280px] md:w-[350px] h-[400px] md:h-[500px] bg-black rounded-[40px] border-8 border-white shadow-2xl overflow-hidden transform rotate-0 md:rotate-[-3deg] md:hover:rotate-0 transition-transform duration-500 z-20">
                           <video autoPlay muted loop playsInline poster="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=800" className="absolute inset-0 w-full h-full object-cover">
                           ...
                           <div className="absolute bottom-8 left-6 right-6">
                                <div className="flex items-center gap-3 mb-4">
                                    <div className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center"><MessageCircle className="w-5 h-5 text-white" /></div>
                                    <div className="h-1 bg-white/20 flex-1 rounded-full overflow-hidden"><div className="h-full bg-white w-1/2 animate-pulse"></div></div>
                                </div>
              ...
              <div className="animate-marquee whitespace-nowrap flex items-center gap-16 px-8 opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
                  {[1,2,3].map(i => (
                    <div key={i} className="flex gap-16 items-center">
                      <LogoTechCrunch /><LogoNYT /><LogoWired /><LogoBloomberg /><LogoForbes />
                    </div>
                  ))}
              </div>
          </div>
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-wrap justify-center gap-6">
                  {featuredSpecialists.map((s, i) => (
                      <div key={i} className="w-full md:w-[31%] bg-white p-4 rounded-3xl shadow-sm border border-yellow-100 hover:shadow-xl transition-all group cursor-pointer transform hover:scale-[1.02]" onClick={onLoginClick}>
                          <div className="aspect-square rounded-2xl overflow-hidden mb-4 relative">
                              ...
                              <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-white backdrop-blur-md ${s.status === 'Available' ? 'bg-green-500/80' : 'bg-yellow-500/80'}`}>{s.status}</div>
                          </div>
          ...
          <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="bg-white rounded-3xl p-8 md:p-12 inline-block max-w-lg w-full relative overflow-hidden shadow-2xl border border-yellow-100">
                  ...
                  <div className="flex items-baseline justify-center gap-4 mb-4">
                      ...
                      <div className="text-6xl md:text-7xl font-black text-black flex items-start gap-1"><span className="text-3xl mt-2">$</span>1.49<span className="text-xl text-gray-500 mt-8">/min</span></div>
                  </div>
                  <p className="text-gray-600 mb-8">Sign up today to grandfather this exclusive rate forever.</p>
                  <button onClick={onLoginClick} className="w-full bg-[#FACC15] text-black py-4 rounded-xl font-bold hover:bg-[#EAB308] transition-transform hover:scale-105 shadow-lg shadow-yellow-500/20">Lock In $1.49 Rate Now</button>
              </div>
          </div>
      </section>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-6 shadow-2xl z-[100] animate-float" style={{animation: 'none'}}>
              <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-start gap-4">
                      <div className="p-3 bg-yellow-50 rounded-xl hidden md:block"><Cookie className="w-6 h-6 text-yellow-600" /></div>
                      <div><h4 className="font-bold text-gray-900">We value your privacy</h4><p className="text-sm text-gray-500 max-w-xl mt-1">We use cookies to enhance your experience. By continuing, you agree to our use of cookies.</p></div>
                  </div>
