    ...
    <div ref={containerRef} className="fixed inset-0 bg-black z-50 flex flex-col overflow-hidden select-none" onMouseMove={handleDrag} onMouseUp={() => setIsDragging(false)} onTouchMove={handleDrag} onTouchEnd={() => setIsDragging(false)}>
        ...
        <div className="absolute top-0 left-0 right-0 p-4 md:p-6 flex justify-between items-start z-20 pointer-events-none bg-gradient-to-b from-black/80 via-black/20 to-transparent pb-20 transition-opacity duration-500">
            <div className="flex items-center gap-4 pointer-events-auto">
                ...
                {lowBalanceWarning && (
                    <div className="bg-red-600 text-white px-4 py-2 rounded-full font-bold text-xs animate-bounce shadow-lg shadow-red-900/50 flex items-center gap-2">
                        <AlertTriangle className="w-4 h-4" /> <span>30s Remaining</span>
                    </div>
                )}
            </div>
            ...
            {connectionState === 'ERROR' && (
                <div className="absolute inset-0 flex flex-col items-center justify-center z-10 bg-black/95">
                    <div className="bg-red-500/10 border border-red-500/30 p-8 rounded-3xl max-w-md text-center backdrop-blur-md">
                        ...
                        <button onClick={onEndSession} className="bg-white text-black px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform flex items-center justify-center gap-2 mx-auto">
                            <RefreshCcw className="w-4 h-4" /> Return to Dashboard
                        </button>
                    </div>
                </div>
            )}
            ...
            {connectionState === 'DEMO_MODE' && (
                <div className="absolute inset-0 w-full h-full bg-black">
                    <img 
                        ...
                        className="w-full h-full object-cover object-top opacity-60 scale-105 animate-pulse-slow" // Slight zoom and pulse for life
                        alt="Background" 
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/40"></div>
                    
                    {/* Simulated Audio Waveform Overlay for Demo */}
            ...
            <div className="absolute inset-0 bg-black">
                {camOn ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-gray-500 bg-gray-900">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest opacity-50">Camera Off</span>
                    </div>
                )}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity duration-200 flex flex-col justify-between p-3">
                    <div className="flex justify-end">
                        <button 
                            ...
                            className="p-1.5 bg-black/50 backdrop-blur-md rounded-lg text-white hover:text-yellow-400 transition-colors"
                        >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 flex justify-center items-end z-30 pointer-events-none bg-gradient-to-t from-black/90 via-black/50 to-transparent h-48">
            <div className="flex items-center gap-3 md:gap-6 pointer-events-auto bg-black/40 backdrop-blur-xl px-6 md:px-8 py-4 rounded-full border border-white/10 shadow-2xl hover:bg-black/50 transition-all transform hover:-translate-y-1">
                ...
                <button 
                    ...
                    className={`p-3 md:p-4 rounded-full transition-all duration-200 ${micOn ? 'bg-gray-800/80 text-white hover:bg-gray-700' : 'bg-red-500 text-white shadow-lg shadow-red-500/30'}`}
                ...
                <button 
                    ...
                    className={`p-3 md:p-4 rounded-full transition-all duration-200 ${camOn ? 'bg-gray-800/80 text-white hover:bg-gray-700' : 'bg-red-500 text-white shadow-lg shadow-red-500/30'}`}
                    title={camOn ? "Turn Off Camera" : "Turn On Camera"}
                >
                    {camOn ? <VideoIcon className="w-5 h-5 md:w-6 md:h-6" /> : <VideoOff className="w-5 h-5 md:w-6 md:h-6" />}
                </button>
                
                <button 
                    onClick={() => setBlurBackground(!blurBackground)} 
                    className={`p-3 md:p-4 rounded-full transition-all duration-200 ${blurBackground ? 'bg-yellow-500 text-black shadow-lg shadow-yellow-500/30' : 'bg-gray-800/80 text-white hover:bg-gray-700'}`}
                ...
                <button 
                    ...
                    className="bg-red-600 hover:bg-red-500 text-white px-6 md:px-8 py-3 md:py-4 rounded-full font-bold flex items-center gap-2 shadow-lg shadow-red-600/20 transition-transform hover:scale-105 active:scale-95"
                >
                    <PhoneOff className="w-5 h-5" />
                    <span className="hidden md:inline tracking-wide">End Session</span>
                </button>
            </div>
        </div>
    </div>
