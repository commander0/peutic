...
import { Heart, Lock, Mail, ArrowRight, Check, Facebook, Twitter, AlertCircle, Shield, Clock, Sparkles, ChevronRight, Key, RefreshCcw } from 'lucide-react';
...
const GOOGLE_CLIENT_ID = "360174265748-nqb0dk8qi8bk0hil4ggt12d53ecvdobo.apps.googleusercontent.com";
const X_CLIENT_ID = "SHk3QkRWY2o0YVMwNUZ6WFllMFQ6MTpjaQ";
const X_CLIENT_SECRET = "z8HCumNixMGf63e7ty1JW3WpwpYGU9p9mcDpO_J34fLfpMVit0";
...
const Auth: React.FC<AuthProps> = ({ onLogin, onCancel }) => {
  ...
  // Initialize Google Sign In with Smart Fallback
  useEffect(() => {
    const handleCredentialResponse = (response: any) => {
        try {
            ...
            // For Google, we skip onboarding for speed in this demo, or could enable it
            onLogin(UserRole.USER, payload.name, payload.picture, payload.email);
        } catch (e) {
            console.error("OAuth Error", e);
            setError("Failed to process Google Sign-In. Using secure fallback...");
            handleSimulatedOAuth('google');
        }
    };
            ...
            try {
                ...
                window.google.accounts.id.renderButton(
                    ...
                    { 
                        theme: "outline", 
                        size: "large", 
                        width: googleBtnRef.current.offsetWidth, // Dynamic width
                        text: "continue_with",
                        shape: "rectangular",
                        logo_alignment: "left"
                    }
                ...
                // Mark as loaded if no immediate error thrown
                setGoogleLoaded(true);
            } catch (err) {
                console.warn("Google Auth Init Failed (likely origin mismatch), using fallback.", err);
    ...
    // Fail-safe: If button is empty after 2 seconds (401 error), force fallback UI
    const fallbackTimer = setTimeout(() => {
        if (googleBtnRef.current && googleBtnRef.current.innerHTML === "") {
            console.log("Google button failed to render (401/403). Switching to manual mode.");
            setGoogleLoaded(false);
        }
    }, 2000);
                ...
                <>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="relative h-[46px] w-full">
                            {/* Actual Google Button Target */}
                            <div 
                                id="googleButton" 
                                ref={googleBtnRef} 
                                className={`absolute inset-0 w-full h-full z-10 ${googleLoaded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                            ></div>

                            {/* Fallback Google Button (Visible if Real Google Fails/Loads Slow) */}
                            <button 
                                type="button"
                            ...
                            >
                                {loading && authProvider === 'google' ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-medium text-sm text-gray-600">Google</span>
                                    </div>
                                )}
                            </button>
                        </div>
